...
namespace MyMath
{
	...
	//ベクトルの引き算
	VECTOR VecSub(VECTOR vecA, VECTOR vecB)
	{
		VECTOR result;
	...
	//2つの座標の距離
	float VecDistance(VECTOR vecA, VECTOR vecB)
	{
	...
	MATRIX MatRotationYawPitchRoll(float yaw, float pitch, float roll)
	{
		MATRIX matYaw = MatRotationYaw(yaw);
		MATRIX matPitch = MatRotationPitch(pitch);
		MATRIX matRoll = MatRotationRoll(roll);

		// 一般的なTPSカメラは Yaw → Pitch → Roll の順で合成
		MATRIX result = MatMult(matYaw, matPitch);
		result = MatMult(result, matRoll);
